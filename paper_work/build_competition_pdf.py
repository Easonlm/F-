...
SOURCE = WORK / "四问正式论文_长稿.md"
HTML = WORK / "四问正式论文_长稿.html"
RAW_PDF = WORK / "四问正式论文_长稿_未编号.pdf"
FINAL_PDF = WORK / "四问正式论文_40页版.pdf"
...
body { color: #000; font-family: SimSun, 'Songti SC', serif; font-size: 12pt;
       line-height: 1.15; text-align: justify; margin: 0; }
...
subprocess.run(
    ["pandoc", str(SOURCE), "--from=markdown+tex_math_dollars+tex_math_single_backslash+pipe_tables",
     "--to=html5", "--standalone", "--mathml", "--output", str(HTML)],
    check=True, cwd=WORK,
)
...
html = re.sub(r'(<h2[^>]*>1 引言</h2>)', r'<div class="body-start"></div>\1', html, count=1)
