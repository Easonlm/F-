...
## 1 问题分析
把能力上升拆为训练规模与控制规模后的时间趋势，并用开放权重模型的能力前沿构造算力放缓情景。由于直接榜单历史只有九个月，贡献与 12/24 月预测均是**探索性结构情景**，没有足够历史作对应期限的回测。
...
## 13 未来12/24月预测
下表是 T1 下的探索性有界前沿；区间是参数 bootstrap 加样本残差的条件区间，未涵盖未来数据源与制度变化。没有12/24个月直接观测回测，不得用作可信的点预测。
...
## 16 不确定性与回测
组外桥接 RMSE {cv_rmse:.2f} 分。可用季度数据只有 {len(ability_q)} 季，直接时间模型 3 个月滚动回测 {len(b3)} 折，RMSE={fnum(b3rmse)} 分；12个月 {len(b12)} 折、24个月 {len(b24)} 折。故长期区间不是经历史覆盖率校准的预测区间。bootstrap 组织重采样、算力与技术情景、桥接分层、问题三外推分别列出，不能将混合范围叫作95%置信区间。
...
"""
    ...
    qa=[
        ...
        f"严格主样本首末季度 p95 实测变化 {r.observed_frontier_change:.2f} 分（下降，且末季 n={int(ability_q.iloc[-1].n)}）。",
        ...
        f"12/24月无有效滚动回测；3月有 {len(b3)} 折，RMSE {fnum(b3rmse)} 分。",
        ...
        "verification 27/30 通过；未通过的是12月/24月直接回测与分解方向一致性。",
    ]
    assert len(qa)==38
...
在项目根目录运行 `python problem4/run_problem4.py`。全部中间表在 `problem4/outputs/tables/`，16张图在 `problem4/outputs/figures/`。`data_pipeline.py` 负责审计、实体匹配、C8解析；`bridge_model.py` 负责桥接；`history_forecast.py` 负责反事实分解、算力和预测；`mechanism.py` 只读问题一至三已有成果；`verify_problem4.py` 输出30项机器检查。未通过的三项属于数据证据不足与模型诊断，不应通过放宽验证标准消除。
"""
