...
问题二保存的最终 P1 V2 在 1M/60M/1B 的中心化斜率分别为 0.864/0.636/0.209，用的是已查看过的测试标签，仅为回顾性证据，不是 λp(N) 的估计。1B 配方均值排序在 V2 下弱于 V1；旧 V1 斜率另存 `p_scale_v1_evidence_audit.csv`。本问以 1M ($N_B=.001$) 为情景归一化点，比较常数 1、幂次衰减 ρ=.1/.2、以及 ρ=.15 且地板 .25。固定 λ 枚举 0、.25、.5、.75、1、1.25、1.5。模型可分意味着 λ>0 时 p 候选排序不变，N-D-Q 不变，收益按 λ 伸缩；λ=0 时所有 p 并列。

| 策略 | λ=1 的 ΔLoss | 变换空间支持等级 | kNN5 | Mahalanobis | PCA 残差 |
|---|---:|---|---:|---:|---:|
| P0_reference | 0 | in-support | 2.1628 | 2.095 | 0.4821 |
| P1_observed_support | -0.4973 | in-support | 2.0865 | 3.5722 | 1.2429 |
| P2_continuous_support | -0.55559 | in-support | 2.0688 | 3.5951 | 1.1414 |
| P3_loose_simplex | -0.89303 | in-support | 2.5034 | 15.097 | 0.86216 |
