...
新增 CLR 加性样条 Ridge，独立选型和保存，不覆盖当前供问题二、三使用的 V2。它在 A4/A5 同划分嵌套交叉验证及既定 1M 测试上改善；60M 为回顾性改善，1B 绝对 Loss RMSE 退步。因此只作为 1M 配比预测的条件候选，不替换跨问 V2 接口。详情见 [V3 对照报告](problem1_v3_report.md)。

在已有 V2 产物基础上，从项目根目录运行：
...
新 artifact 为 `outputs/models/mixture_v3_1m.joblib`。其 `model` 接收 `clr(p + eps)` 特征，调用 `problem1_v3.predict_v3(bundle, p)` 可直接输入按 artifact `pcols` 排序的配比矩阵。训练选择、嵌套折、跨规模与逐领域对照均在 `outputs/tables/v3_*.csv`；核验见 [V3 核验](verification_report_v3.md)。
...
主流程另输出 `outputs/tables/v2_scale_transfer_summary.csv` 与 `v2_scale_mixture_predictions.csv`：用最终 V2 在既定 1M/60M/1B 实测配方上诊断幅度、排序和偏差。1B 的配方均值排序弱于 V1；这些测试标签不用于拟合跨规模系数。
