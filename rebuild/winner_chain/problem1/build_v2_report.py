...
def run():
    ...
    scale = pd.read_csv(TABLES / "v2_scale_transfer_summary.csv")
    ...
    compare_lines += ["", "## 最终配比模型的跨规模幅度诊断", "",
        ...
        "|---|---:|---:|---:|---:|"]
    for dataset in ["test_1m", "test_60m", "test_1B"]:
        z = scale[scale.dataset.eq(dataset)].set_index("version")
        v1, v2 = z.loc["v1"], z.loc["v2"]
        compare_lines.append(f"| {dataset} | {v1.centered_slope:.3f} | {v2.centered_slope:.3f} [{v2.slope_ci_low:.3f}, {v2.slope_ci_high:.3f}] | {v1.mean_loss_spearman:.3f}/{v2.mean_loss_spearman:.3f} | {v2.mean_loss_bias:+.3f} |")
    compare_lines += ["",
        "## 未解决的误差", "",
