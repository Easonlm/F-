...
def p_scale_evidence():
    """Preserve V1 provenance and publish the final P1 V2 scale diagnostic."""
    ...
    final=pd.read_csv(ROOT/'problem1/outputs/tables/v2_scale_transfer_summary.csv')
    final=final[final.version=='v2'].copy()
    final=final.rename(columns={'slope_ci_low':'ci_low','slope_ci_high':'ci_high',
                                'mean_loss_spearman':'spearman'})
    final['source']='problem1 v2 final artifact; retrospective existing test labels; not identified lambda_p(N)'
    save(final[['dataset','n_mixtures','centered_slope','ci_low','ci_high','spearman',
                'mean_loss_bias','evidence_status','source']],
         'p_scale_transfer_from_problem1_v2.csv')
    return final
...
'''
    ...
    pscale=pd.read_csv(T/'p_scale_transfer_from_problem1_v2.csv')
...
'''
    supplement=HERE/'source_quality_calibration_addendum.md'
...
def verify(d,mono,over,ndscore,qscore):
    ...
    p1_scale=pd.read_csv(ROOT/'problem1/outputs/tables/v2_scale_transfer_summary.csv')
    p1_scale=p1_scale[p1_scale.version=='v2'].sort_values('dataset')
    p2_scale=pd.read_csv(T/'p_scale_transfer_from_problem1_v2.csv').sort_values('dataset')
    checks['final_P1_V2_scale_evidence']=len(p1_scale)==len(p2_scale)==3 and np.allclose(
        p1_scale.centered_slope.to_numpy(float),p2_scale.centered_slope.to_numpy(float),atol=1e-9)
