...
def run():
    ...
    scale_predictions, scale_summary = [], []
    for label, prefix, scale in [("test_1m", "test", "1m"), ("test_60m", "test", "60m"),
        ...
        for ver, pred in [("v1", pred1), ("v2", pred2)]:
            ...
            if label.startswith("test_"):
                actual_mean = yt.mean(axis=1)
                predicted_mean = pred.mean(axis=1)
                slope = float(np.cov(predicted_mean, actual_mean, ddof=0)[0, 1] / np.var(predicted_mean))
                rng = np.random.default_rng(20260924 + {"test_1m": 1, "test_60m": 2, "test_1B": 3}[label])
                slopes = []
                for _ in range(3000):
                    sampled = rng.integers(len(yt), size=len(yt))
                    x, target = predicted_mean[sampled], actual_mean[sampled]
                    if np.var(x) > 1e-12:
                        slopes.append(float(np.cov(x, target, ddof=0)[0, 1] / np.var(x)))
                scale_summary.append({"dataset": label, "version": ver, "n_mixtures": len(yt),
                ...
                scale_predictions.extend({"dataset": label, "version": ver, "index": idx,
    ...
    pd.DataFrame(scale_predictions).to_csv(TABLES / "v2_scale_mixture_predictions.csv", index=False)
    pd.DataFrame(scale_summary).to_csv(TABLES / "v2_scale_transfer_summary.csv", index=False)
