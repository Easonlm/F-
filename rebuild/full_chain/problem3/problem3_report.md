...
P1 训练样本支持方案使模型预测 Loss 相对 p_ref 下降 0.502，但这一数量级受未识别 λ_p 及跨模型尺度转移支配，论文建议把 p_ref 作为主预测基线，将 P1 作为同算力配比重分配候选。Loose 的 1.195 改善来自连续 simplex 数学优化；即便最近邻指标未越过 95% 局部阈值，该点并非观测训练配比，仍需新的联合实验验证。
