...
def mixture_work(artifact,p,old_mix,v1):
    ...
    evidence=pd.read_csv(ROOT/'problem2_v2/outputs/tables/p_scale_transfer_from_problem1.csv')
    ...
    write(evidence,'p_scale_evidence_audit.csv');write(prev,'p_scale_v1_scenario_audit.csv')
...
问题二保存的 1M/60M/1B 中心化斜率分别为 0.900、0.658、0.285，来源是问题一 V1 的 alr_ridge 测试预测，仅为探索性证据，不是 λp(N) 的估计。V2 以 1M ($N_B=.001$) 为情景归一化点，比较常数 1、幂次衰减 ρ=.1/.2、以及 ρ=.15 且地板 .25。固定 λ 枚举 0、.25、.5、.75、1、1.25、1.5。模型可分意味着 λ>0 时 p 候选排序不变，N-D-Q 不变，收益按 λ 伸缩；λ=0 时所有 p 并列。
...
'''
    ...
    (HERE/'README.md').write_text('# Problem 3 V2\n\nRun `python problem3_v2/run_v2.py` from the repository root. V1 is read-only and its SHA-256 integrity is checked. See `problem3_v2_report.md`, `问题三第二版完整详解.md`, and `outputs/tables/verification_results.json`.\n',encoding='utf-8')
